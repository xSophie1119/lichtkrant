# P2000 Monitor — Windows + Linux v4.5.1
...
## Nieuw in v4.5.0 — Windows-herstel, betrouwbare omroep en platformmappen

- Windows-kiosks zijn versiegebonden. Een oude Edge/Chrome-kiosk van een vorige release wordt bij starten vervangen; een gezonde kiosk van dezelfde versie blijft gewoon draaien.
- De kiosk-URL bevat een versiebuster en het browserprofiel bevat het versienummer, zodat een nieuwe backend nooit meer ongemerkt met oude frontend-JavaScript draait.
- Windows-omroep heeft een browser-onafhankelijke localhost-fallback: als Chromium audio blokkeert of stil blijft, speelt Windows de lokaal gegenereerde Nederlandse dispatch-WAV zelf af via `System.Media.SoundPlayer`.
- Host-TTS is alleen vanaf localhost toegestaan, zodat een beheercomputer op het LAN nooit per ongeluk zelf gaat spreken.
- Voertuigen worden uitgesproken met hun **volledige functie + standplaats** uit de SW Mediaproducties Roepnummer API. Bijvoorbeeld `20-9432` → **Tankautospuit Tilburg Vossenberg**; het numerieke roepnummer zelf wordt niet uitgesproken. De beheerpreview gebruikt exact dezelfde voertuigservice.
- De distributie is opgesplitst in complete `Windows/` en `Linux/` mappen. Iedere map kan zelfstandig worden gekopieerd/gebruikt. De updater kan zo'n gecombineerde ZIP per platform uitpakken.
...
- Na iedere start wordt databasehistorie alleen als baseline ingelezen; uitsluitend daarna ontvangen meldingen worden getoond en uitgesproken.
- De Linux/Windows-supervisor wacht langer op trage processen, herstart één keer per stale-heartbeatepisode en begrenst automatische herstartpogingen.
- Alle zichtbare P2000-regels worden ontdaan van ritnummers, capcodes, postcodes, URL- en RSS-ruis.
- De incidentkaart toont incident én ingestelde standplaats, hemelsbrede afstand en een knop voor de autoroute.
- Volume `0` is werkelijk stil; mastervolume werkt lineair en is rechtstreeks vanaf het meldingsscherm te bedienen.
