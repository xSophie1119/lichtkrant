...
def compile_case():
    for rel in ['backend/remote_control.py','backend/remote_storage.py','backend/remote_api.py','backend/remote_dashboard.py','tools/supervisor_commands.py','backend/server.py','tools/health_gate.py','tools/recovery_bootstrap.py','tools/startup_guard.py','tools/supervisor.py','tools/linux_desktop.py','tools/rollback_latest.py','tools/stop_verify.py','tools/run_tests.py']:
...
def health_case():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for rel in h.REQUIRED_FRONTEND:(r/rel).write_text('x'*20)
...
def recovery_marker_case():
    ...
    with tempfile.TemporaryDirectory() as td:
        r=Path(td);b=r/'backup';b.mkdir();(b/'VERSION').write_text('old');p=r/'pending.json';j=r/'journal.json';p.write_text(json.dumps({'backup':str(r/'missing')}));m.ROOT=r;m.PENDING=p;m.JOURNAL=j;out=m.recover_pending();req(not out['ok'] and p.exists(),'failed rollback destroyed marker');meta=json.loads(p.read_text(encoding='utf-8'));req(meta['attempt_count']==1 and meta['last_error'],'failure state missing')
def mirror_case():
    m=load('rb2',ROOT/'tools/recovery_bootstrap.py')
    with tempfile.TemporaryDirectory() as td:
        r=Path(td);b=r/'backup';d=r/'dst';b.mkdir();d.mkdir();(b/'VERSION').write_text('old');(b/'keep.txt').write_text('yes');(d/'obsolete.txt').write_text('no');(d/'VERSION').write_text('new');m.mirror_restore(b,d,preserve=());req((d/'VERSION').read_text(encoding='utf-8')=='old' and not (d/'obsolete.txt').exists(),'mirror rollback not exact')
def startup_static_case():
    for p in [ROOT/'START_P2000.sh',ROOT/'START_P2000.bat']:
        t=p.read_text(encoding='utf-8');req('startup_guard.py' in t and '--kill-stale' not in t,'launcher still owns lifecycle transaction')
    g=(ROOT/'tools/startup_guard.py').read_text(encoding='utf-8')
...
def stop_static_case():
    for p in [ROOT/'STOP_P2000.sh',ROOT/'STOP_P2000.bat']:
        req('stop_verify.py' in p.read_text(encoding='utf-8'),'STOP has no verification')
def security_static_case():
    t=(ROOT/'backend/server.py').read_text(encoding='utf-8');req('127.0.0.1' in t and 'Handmatige executable ZIP-updates zijn uitgeschakeld' in t,'security guard absent')
    t=(ROOT/'backend/remote_control.py').read_text(encoding='utf-8');req('X-P2000-Admin-Token' in t and 'SameSite=Strict' in t,'pairing guards absent')
...
def browser_sources_case():
    t=(ROOT/'tools/linux_desktop.py').read_text(encoding='utf-8');req("snap,'run','chromium'" in t and "flat,'run','org.chromium.Chromium'" in t,'snap/flatpak missing')
def concurrency_case():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        a=subprocess.Popen([sys.executable,'-c',code]);b=subprocess.Popen([sys.executable,'-c',code]);a.wait(8);b.wait(8);req(a.returncode==0 and b.returncode==0,'concurrent workers failed');rows=[float(x) for x in out.read_text(encoding='utf-8').splitlines()];req(len(rows)==2 and abs(rows[1]-rows[0])>.55,'startup mutex did not serialize')
def runtime_case():
    result=subprocess.run([sys.executable,'-m','unittest','discover','-s',str(ROOT/'tests'),'-p','test_*.py','-v'],cwd=ROOT,timeout=60)
    req(result.returncode==0,'runtime regression suite failed')
CASES=[runtime_case,compile_case,shell_case,health_case,manifest_case,recovery_marker_case,mirror_case,startup_static_case,stop_static_case,security_static_case,linux_handoff_case,browser_sources_case,concurrency_case]
def main():
    fail=0
    for fn in CASES:
        st=time.monotonic()
