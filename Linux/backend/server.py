...
def parser_text_diagnostics(raw: str, parsed: dict, state: "AppState" | None = None) -> dict:
    ...
    if allowed and units:
        ...
        for unit in units[:12]:
            ...
            if state is not None and digits:
                try:
                    ...
                    meta = None
            code = normalize_space(str((meta or {}).get("type") or (meta or {}).get("function_code") or "")).upper()
            label = normalize_space(str((meta or {}).get("function_name") or (meta or {}).get("label") or ""))
            station = normalize_space(str((meta or {}).get("station_name") or (meta or {}).get("station") or ""))
        ...
        if spoken_units:
            joined = spoken_units[0] if len(spoken_units) == 1 else (
                f"{spoken_units[0]} en {spoken_units[1]}" if len(spoken_units) == 2
                else f"{', '.join(spoken_units[:-1])} en {spoken_units[-1]}"
            )
            speech += f" Gealarmeerde voertuigen: {joined}."
    if allowed:
...
class AppState:
    ...
    def _save_display_settings_locked(self, payload: dict) -> dict:
        ...
        if "mapMode" in clean:
            ...
            clean["mapMode"] = mode if mode in {"route", "incident"} else "route"
...
def _validate_and_extract_update(zip_path: Path) -> tuple[Path, str]:
    ...
    # Accept both a traditional single application folder and the v4.4.19
    ...
    if len(roots) == 1:
        ...
        # v4.4.19 bundles may deliberately contain separate Windows/ and Linux/
