...
function vehicleSpeechLabel(v){
  ...
  const expanded={
    // Kazerneomroep: spreek TS bewust als letters uit ("T S") in plaats van
    // het type stil te normaliseren naar alleen "tankautospuit".
    'TS':'T S','TS-RES':'reserve T S','HW':'hoogwerker','AL':'autoladder',
    ...
    'TS-WS':'T S met waterscherm','TST-NB':'T S natuurbrandbestrijding','HW-32':'hoogwerker','HA':'haakarmvoertuig',
  ...
  if(type==='TS'&&/reserve/i.test(String(v.label||'')))label='reserve T S';
...
function vehicleSpeechPhrase(m){
  ...
  const parts=[];
  if(counts.ts)parts.push(counts.ts===1?'één T S':`${({2:'twee',3:'drie',4:'vier',5:'vijf',6:'zes',7:'zeven',8:'acht',9:'negen',10:'tien',11:'elf',12:'twaalf'})[counts.ts]||counts.ts} T S voertuigen`);
