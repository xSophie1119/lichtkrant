...
DEFAULT_GITHUB_MANIFEST_PATH = "update-manifest.json"
SETTINGS_BACKUP_DIR = DATA_DIR / "settings-backups"
INCIDENT_CORRECTIONS_PATH = DATA_DIR / "incident-corrections.json"
STARTUP_STATE_PATH = DATA_DIR / "startup-state.json"
SAFE_MODE = False
SAFE_MODE_REASON = ""
...
from core.config_schema import CONFIG_SCHEMA_VERSION, DISPLAY_SCHEMA_VERSION, migrate_config, migrate_display
from core.incidents import incident_similarity
from core.priority import priority_tier
from core.speech_templates import build_announcement, clean_station, join_spoken_units
from core.updater_manifest import parse_manifest, platform_key as update_platform_key, verify_file as verify_update_file
...
def _api_mapping(value) -> dict | None:
...
def _api_text(value, *, nested_keys: tuple[str, ...] = ("name", "label", "detail", "code", "city", "value"), limit: int = 240) -> str:
    ...
    # Last guard: object-ish data is metadata, never speech/display text.
...
def _pick_text(row: dict, *keys: str, limit: int = 240, nested_keys: tuple[str, ...] = ("name", "label", "detail", "code", "city", "value")) -> str:
    for key in keys:
        text = _api_text(row.get(key), nested_keys=nested_keys, limit=limit)
        if text:
            return text
    return ""
...
def canonical_sw_unit(row: dict) -> tuple[str, dict] | tuple[None, None]:
    ...
    if not lookup_key:
        return None, None
    function_code = _pick_text(row, "function_code", "functionCode", "type_code", "type", "function", limit=64, nested_keys=("code", "function_code", "type_code", "name"))
    ...
    for key in ("station_code", "city", "function_detail", "notes", "source_name", "source_url"):
        ...
        if value:
            canonical[key] = value
    station_obj = _api_mapping(row.get("station")) or {}
...
def load_sw_vehicle_cache() -> tuple[dict[str, dict], dict]:
    try:
        ...
        for key, row in units.items():
            ...
            if lookup:
                _normalized_key, normalized_row = canonical_sw_unit(dict(row))
...
def sw_units_to_vehicle_catalog(units: dict[str, dict]) -> dict[str, dict]:
    ...
    for lookup, unit in units.items():
        ...
        if len(digits) not in {5, 6, 7}:
            continue
        function_code = _api_text(unit.get("function_code"), nested_keys=("code", "function_code", "type_code", "name"), limit=64)
...
def parser_text_diagnostics(raw: str, parsed: dict, state: "AppState" | None = None) -> dict:
    ...
    # Keep the beheer-preview identical to the real kazerneomroep. Callsigns
    # themselves stay out of speech; the full vehicle function and station are
    # spoken instead, e.g. 20-9432 -> "Tankautospuit Tilburg Vossenberg".
    if allowed and units:
        for unit in units[:12]:
            digits = normalize_vehicle_digits(unit)
            meta = None
            if state is not None and digits:
                try:
                    with state.vehicle_catalog_lock:
                        meta = dict(state.vehicle_catalog.get(digits) or {})
                except Exception:
                    meta = None
            code = _api_text((meta or {}).get("type") or (meta or {}).get("function_code"), nested_keys=("code", "function_code", "type", "name"), limit=64).upper()
            ...
            if not code and digits:
                code, inferred_label = fire_vehicle_type(label, digits)
                if not label:
                    label = inferred_label
            expanded = {
                "TS": "Tankautospuit", "TS-RES": "Reserve tankautospuit",
                "TS-WS": "Tankautospuit met waterscherm", "TST-NB": "Tankautospuit natuurbrandbestrijding",
                "HW": "Hoogwerker", "AL": "Autoladder", "RV": "Redvoertuig",
                "HV": "Hulpverleningsvoertuig", "HVT-KR": "Hulpverleningsvoertuig met kraan",
                "OVD-B": "Officier van Dienst Brandweer", "HOVD-B": "Hoofdofficier van Dienst Brandweer",
                "AGS": "Adviseur Gevaarlijke Stoffen", "VEBS": "Verkenningseenheid Brandweer",
                "WT": "Watertankwagen", "WTS": "Watertransportsysteem", "WTH": "Watertankhaakarmbak",
                "SB": "Schuimblusvoertuig", "WO": "Waterongevallenvoertuig",
            }
            base = label or expanded.get(code, "")
            if not base and code:
                # Keep unknown but valid function codes useful without ever reading
                # the numerical callsign itself.
                base = expanded.get(code, code)
            # Station names are often stored as Tilburg-Vossenberg; a natural
            # spoken pause/space sounds better than reading the hyphen literally.
            station_speech = normalize_space(re.sub(r"\s*[-–—]\s*", " ", station))
            base_speech = normalize_space(re.sub(r"\s*[-–—]\s*", " ", base))
            spoken = base_speech
            if station_speech and normalize_city_token(station_speech) not in normalize_city_token(base_speech):
                spoken = normalize_space(f"{base_speech} {station_speech}")
            if spoken and spoken.casefold() not in {x.casefold() for x in spoken_units}:
                spoken_units.append(spoken)
    templates = {}
    if state is not None:
        try: templates = dict(state.get_display_settings().get("speechTemplates") or {})
        except Exception: templates = {}
    speech = build_announcement(incident=speech_incident, where=_speech_where(city, location), scale=scale,
                                units=spoken_units, mmt=detect_mmt_resource(raw, "", units), templates=templates)
    speech = normalize_space(speech)
    if allowed:
...
def parse_raw_p2000_line(state: "AppState", raw: str, categories: list[str] | None = None) -> dict:
    ...
    result.update(parser_text_diagnostics(title, result, state))
    return result
...
class AppState:
    def __init__(self, config: dict):
        ...
        self.settings_cache_at = 0.0
        self.health_gate_lock = threading.Lock()
        self.health_gate_cache = None
        self.health_gate_at = 0.0
    ...
    def get_display_settings(self) -> dict:
        # Return independent objects: callers may edit a list before a partial save.
        with self.config_lock:
            now = time.monotonic()
            if self.settings_cache is None or now - self.settings_cache_at >= 2.0:
                self.settings_cache = self._load_display_settings()
                self.settings_cache_at = now
            return copy.deepcopy(self.settings_cache)

    def _load_display_settings(self) -> dict:
        value = self._read_display_settings_db()
        value, migration_notes = migrate_display(value)
        # Persist schema migrations without recursively calling save_display_settings.
        if migration_notes:
            try:
                with self.connect() as con:
                    con.execute("INSERT INTO kv(key,value) VALUES('display:settings',?) ON CONFLICT(key) DO UPDATE SET value=excluded.value",
    ...
    def _save_display_settings_locked(self, payload: dict, *, replace: bool = False) -> dict:
        allowed = {
            ...
            "mapEnabled", "mapZoom", "mapMode", "locationAliases", "ttsDictionary", "capcodeMap",
            "speechTemplates", "displayTarget", "settingsSchemaVersion",
        ...
        # v4.5 fixes the primary speech engine per installation. Native OS audio
        # is authoritative; browser speech is emergency fallback only.
        if "speechEngine" in clean:
            engine=str(clean["speechEngine"] or "native").lower()
            clean["speechEngine"] = engine if engine in {"native","piper","espeak","sapi","online"} else "native"
        ...
        for dict_key in ("locationAliases", "ttsDictionary", "capcodeMap", "speechTemplates"):
            if dict_key in clean:
                if not isinstance(clean[dict_key], dict):
                    ...
                    for i, (k, v) in enumerate(clean[dict_key].items()):
                        ...
                        elif dict_key == "speechTemplates":
                            if kk in {"incident","scale","units","mmt"}:
                                vv=str(v)[:300].strip()
                                if vv: clipped[kk]=vv
                        else:
                            vv = str(v)[:160].strip()
        ...
        if "mapMode" in clean:
            mode = normalize_space(str(clean.get("mapMode") or "route")).lower()
            clean["mapMode"] = mode if mode in {"auto", "route", "incident", "posts"} else "auto"
        if "displayTarget" in clean:
            clean["displayTarget"] = normalize_space(str(clean["displayTarget"] or "all"))[:120] or "all"
        clean["settingsSchemaVersion"] = DISPLAY_SCHEMA_VERSION
        ...
        if clean.get("kioskMonitor") != existing.get("kioskMonitor"):
            self.display_info_cache = None
            self.display_info_monotonic = 0.0
        self.settings_cache = copy.deepcopy(clean)
        self.settings_cache_at = time.monotonic()
    ...
    def lookup_cached_geocode(self, city: str, location: str) -> dict | None:
        ...
        return {
            ...
            "source": row["source"],
            "confidence": {"pdok-location":0.99,"pdok":0.98,"bgt-straatindex":0.97,"nominatim":0.82}.get(str(row["source"] or "").lower(),0.90),
            "query": normalize_space(f"{row['location']} {row['city']}"),
    ...
    def store_geocode(self, city: str, location: str, payload: dict) -> dict:
        ...
        out = {**payload, "lookup_key": key, "city": city, "location": location, "cached_at": now, "cached": False,
               "confidence": float(payload.get("confidence") or {"pdok-location":0.99,"pdok":0.98,"bgt-straatindex":0.97,"nominatim":0.82}.get(source,0.88)),
               "query": normalize_space(str(payload.get("query") or f"{location} {city}")),
               "canonical_location": normalize_space(location)}
        return out
    ...
    def display_clients_view(self, max_age_seconds: float = 45.0) -> list[dict]:
        cutoff=time.monotonic()-max(5.0,float(max_age_seconds)); now=time.monotonic(); rows=[]
        with self.display_clients_lock:
            for cid,row in self.display_clients.items():
                seen=float(row.get("seen_monotonic") or 0); clean={k:v for k,v in row.items() if k!="seen_monotonic"}
                clean["online"]=seen>=cutoff;clean["heartbeat_age_seconds"]=round(max(0,now-seen),1);clean["client_id"]=cid
                rows.append(clean)
        rows.sort(key=lambda r:(bool(r.get("online")),str(r.get("display_name") or r.get("client_id"))),reverse=True)
        return rows
    ...
    def health_snapshot(self, force: bool = False) -> dict:
        ...
        result = {
            ...
            "displays": self.display_clients_view(),
...
def message_debug_view(state: AppState, message_id: str) -> dict:
    message_id=normalize_space(str(message_id or ""))[:240]
    rows=query_messages(state,{"limit":["500"]})
    m=next((x for x in rows if str(x.get("id"))==message_id),None)
    if not m: raise ValueError("Melding niet gevonden")
    parsed=parse_raw_p2000_line(state,m.get("title") or m.get("summary") or "",m.get("categories") or [])
    vehicle_rows=[]
    with state.vehicle_catalog_lock:
        for unit in parsed.get("units") or []:
            digits=normalize_vehicle_digits(unit);meta=dict(state.vehicle_catalog.get(digits) or {}) if digits else {}
            vehicle_rows.append({"callsign":unit,"lookup_key":digits,"found":bool(meta),"source":meta.get("source") or meta.get("source_name") or "fallback",
                                 "function_code":meta.get("function_code") or meta.get("type") or "","function_name":meta.get("function_name") or meta.get("label") or "",
                                 "station_name":meta.get("station_name") or meta.get("station") or "","discipline":meta.get("discipline") or ""})
    incident_rows=build_incidents(rows,100)
    incident=next((x for x in incident_rows if any(str(mm.get("id"))==message_id for mm in x.get("messages",[]))),None)
    map_info=None;map_error=""
    try: map_info=state.geocode_incident(parsed.get("city",m.get("city","")),parsed.get("location",m.get("location","")),14)
    except Exception as exc: map_error=str(exc)
    return {"message":m,"pipeline":{"raw":m.get("title") or "","discipline":parsed.get("service"),"priority":parsed.get("priority"),
            "parser":parsed,"vehicles":vehicle_rows,"incident":incident,"map":map_info,"map_error":map_error,
            "screen_text":parsed.get("screen_text"),"speech_text":parsed.get("speech_text"),"removed":parsed.get("removed") or [],
            "unit_policy":parsed.get("unit_policy")},"generated_at":utcnow_iso()}


def map_context_view(state: AppState, message_id: str, zoom: int = 14) -> dict:
    rows=query_messages(state,{"id":[str(message_id)],"limit":["1"]});m=rows[0] if rows else None
    if not m: raise ValueError("Melding niet gevonden")
    incident=state.geocode_incident(m.get("city",""),m.get("location",""),zoom)
    posts=[];seen=set()
    with state.vehicle_catalog_lock:
        catalog=dict(state.vehicle_catalog)
    for unit in m.get("units") or []:
        digits=normalize_vehicle_digits(unit);meta=dict(catalog.get(digits) or {}) if digits else {}
        station=normalize_space(str(meta.get("station_name") or meta.get("station") or ""));city=normalize_space(str(meta.get("city") or ""))
        if not station: continue
        key=(normalize_city_token(city),normalize_location_token(station))
        if key in seen:continue
        seen.add(key)
        try:
            geo=state.geocode_incident(city or m.get("city",""),station,12)
            posts.append({"name":station,"city":city,"lat":geo["lat"],"lon":geo["lon"],"source":geo.get("source"),"confidence":geo.get("confidence")})
        except Exception:pass
        if len(posts)>=8:break
    return {"incident":incident,"posts":posts,"message_id":m.get("id"),"generated_at":utcnow_iso()}


def _read_incident_corrections() -> dict:
    try:
        row=json.loads(INCIDENT_CORRECTIONS_PATH.read_text(encoding="utf-8"))
        return row if isinstance(row,dict) else {"history":[]}
    except Exception:return {"history":[]}

def _append_incident_correction(row: dict) -> None:
    doc=_read_incident_corrections(); history=doc.get("history") if isinstance(doc.get("history"),list) else []
    history.append({**row,"at":utcnow_iso()});doc["history"]=history[-500:]
    INCIDENT_CORRECTIONS_PATH.parent.mkdir(parents=True,exist_ok=True)
    tmp=INCIDENT_CORRECTIONS_PATH.with_suffix(".tmp");tmp.write_text(json.dumps(doc,ensure_ascii=False,indent=2),encoding="utf-8");tmp.replace(INCIDENT_CORRECTIONS_PATH)


def apply_incident_correction(state: AppState, payload: dict) -> dict:
    action=normalize_space(str(payload.get("action") or "")).lower()
    if action=="merge":
        ids=[normalize_space(str(x)) for x in payload.get("message_ids",[]) if normalize_space(str(x))][:100]
        if len(ids)<2: raise ValueError("Selecteer minimaal twee meldingen")
        target=normalize_space(str(payload.get("incident_key") or "manual-"+hashlib.sha256("|".join(sorted(ids)).encode()).hexdigest()[:20]))[:180]
        with state.connect() as con:
            con.executemany("UPDATE messages SET incident_key=? WHERE id=?",[(target,x) for x in ids])
        _append_incident_correction({"action":"merge","message_ids":ids,"incident_key":target})
        return {"ok":True,"action":action,"incident_key":target,"changed":len(ids)}
    if action in {"edit","move"}:
        mid=normalize_space(str(payload.get("message_id") or ""))[:240]
        if not mid: raise ValueError("message_id ontbreekt")
        fields={};args=[]
        for key in ("city","location","incident_key"):
            if key in payload:
                fields[key]=normalize_space(str(payload.get(key) or ""))[:240]
        if not fields: raise ValueError("Geen correctie opgegeven")
        sql="UPDATE messages SET "+", ".join(f"{k}=?" for k in fields)+" WHERE id=?";args=list(fields.values())+[mid]
        with state.connect() as con: cur=con.execute(sql,args)
        _append_incident_correction({"action":action,"message_id":mid,"fields":fields})
        return {"ok":True,"action":action,"changed":int(cur.rowcount or 0),"fields":fields}
    raise ValueError("Onbekende incidentcorrectie")
...
def incident_urgency_details(chunk: list[dict]) -> tuple[int, list[str], str]:
    """Stable tier + explainable score. The tier drives behaviour; score only sorts within it."""
    if not chunk: return 0, [], "low"
    ...
    special=bool(re.search(r"\b(?:HVT-KR|HV-K|KRAAN|COBRA|BLUSROBOT|WTS|WTH|WATERTRANSPORT|SB|SCHUIMBLUS|STH|USAR|AGS|VEBS|FO[- ]?VOERTUIG)\b",joined,re.I))
    max_scale=max((int(m.get("scale_score") or 0) for m in chunk),default=0)
    base=max((_priority_score(m.get("priority", "")) for m in chunk),default=0)*8 + (30+min(45,max_scale*8) if max_scale else 0)
    details=priority_tier(classification=classification,text=joined,scale=" ".join(str(m.get("scale") or "") for m in chunk),
                          mmt=detect_mmt_resource(joined,"",[]),message_count=len(chunk),special_vehicle=special,base_score=base)
    reasons=list(details["reasons"])
    if max_scale and "opschaling" not in reasons: reasons.insert(0,"opschaling")
    services={normalize_space(str(m.get("service") or "")) for m in chunk if normalize_space(str(m.get("service") or ""))}
    if len(services)>=2:
        details["score"]=min(999,int(details["score"])+min(28,10+(len(services)-2)*6));reasons.append("meerdere disciplines")
    return int(details["score"]), list(dict.fromkeys(reasons)), str(details["tier"])


def _incident_chunk_summary(key: str, chunk: list[dict], now: datetime, similarity_notes: list[str] | None = None) -> dict:
    chunk=sorted(chunk,key=lambda m:m["published"])
    ...
    urgency_score,urgency_reasons,tier=incident_urgency_details(chunk)
    return {
        "id":f"{key}-{first['published'][:16]}","incident_key":key,"first_seen":first["published"],"last_seen":latest["published"],
        "age_seconds":age_seconds,"active":age_seconds<=1800,"classification":incident_classification(chunk),"title":latest["title"],
        "city":latest["city"],"location":latest["location"],"priority":latest["priority"],"services":services,"units":units,
        "message_count":len(chunk),"scale":highest.get("scale",""),"scale_score":highest.get("scale_score",0),
        "parser_confidence":int(latest.get("parser_confidence") or 0),"parser_confidence_avg":round(sum(confidences)/len(confidences)) if confidences else 0,
        "timeline":timeline,"latest_delta":latest_delta,"priority_tier":tier,"urgency_score":urgency_score,"urgency_reasons":urgency_reasons,
        "similarity_notes":list(dict.fromkeys(similarity_notes or [])),"messages":list(reversed(chunk))[:40],"source_url":latest["url"],
    }


def build_incidents(messages: list[dict], limit: int = 20) -> list[dict]:
    """Incident engine: exact keys first, then explainable near-duplicate merging.

    This turns follow-up messages/scale changes into one incident even when the raw
    wording changed enough to produce a different legacy incident_key.
    """
    groups:dict[str,list[dict]]={}
    for m in messages: groups.setdefault(m["incident_key"],[]).append(m)
    chunks:list[tuple[str,list[dict]]]=[]
    for key,rows in groups.items():
        rows.sort(key=lambda m:m["published"]); current=[]; prev=None
        for m in rows:
            dt=datetime.fromisoformat(m["published"].replace("Z","+00:00"));
            if dt.tzinfo is None:dt=dt.replace(tzinfo=timezone.utc)
            if prev and (dt-prev).total_seconds()>1800:
                if current:chunks.append((key,current))
                current=[]
            current.append(m);prev=dt
        if current:chunks.append((key,current))
    # Newest chunks claim older similar chunks. Require the explainable matcher,
    # so same-place but different-street calls do not get merged accidentally.
    chunks.sort(key=lambda x:x[1][-1]["published"],reverse=True)
    merged:list[dict]=[]
    for key,rows in chunks:
        candidate={**rows[-1],"classification":incident_classification(rows),"last_seen":rows[-1]["published"]}
        best=None;best_sim=None
        for existing in merged:
            probe={"city":existing["rows"][-1].get("city",""),"location":existing["rows"][-1].get("location",""),
                   "classification":incident_classification(existing["rows"]),"last_seen":existing["rows"][-1].get("published",""),
                   "units":[u for m in existing["rows"] for u in (m.get("units") or [])]}
            sim=incident_similarity(candidate,probe)
            if sim.get("match") and (best_sim is None or sim["score"]>best_sim["score"]):best,best_sim=existing,sim
        if best is None:
            merged.append({"key":key,"rows":list(rows),"notes":[]})
        else:
            best["rows"].extend(rows);best["rows"].sort(key=lambda m:m["published"])
            best["notes"].append(f"samengevoegd ({best_sim['score']}%): "+", ".join(best_sim.get("reasons") or []))
    now=datetime.now(timezone.utc)
    incidents=[_incident_chunk_summary(x["key"],x["rows"],now,x["notes"]) for x in merged]
    tier_rank={"critical":4,"high":3,"normal":2,"low":1}
    incidents.sort(key=lambda i:(bool(i["active"]),tier_rank.get(i.get("priority_tier"),0),int(i.get("urgency_score") or 0),i["last_seen"]),reverse=True)
...
def _piper_config() -> tuple[str | None, Path | None]:
    if not sys.platform.startswith("linux"):
        return None, None
    exe = shutil.which("piper")
    if not exe:
        return None, None
    configured = normalize_space(os.environ.get("P2000_PIPER_MODEL", ""))
    candidates=[]
    if configured: candidates.append(Path(configured))
    voices = DATA_DIR / "voices"
    if voices.exists():
        candidates.extend(sorted(voices.glob("*nl*.onnx")))
        candidates.extend(sorted(voices.glob("*.onnx")))
    for model in candidates:
        if model.is_file(): return exe, model.resolve()
    return exe, None


def generate_local_piper_wav(text: str, rate: float = 0.96, service: str = "brandweer", urgent: bool = False, attention: bool = True) -> bytes:
    """Stable offline Linux voice when a Dutch Piper ONNX model is installed."""
    global _TTS_RENDER_LAST_ENGINE, _TTS_RENDER_LAST_ERROR, _TTS_RENDER_LAST_AT, _TTS_RENDER_LAST_VOICE
    text=normalize_space(str(text or ""))[:1200]
    ...
    with _TTS_RENDER_LOCK:
        ...
        try:
            ...
            _TTS_RENDER_LAST_ENGINE="linux-piper-wav"; _TTS_RENDER_LAST_VOICE=f"Piper • {model.stem}"; _TTS_RENDER_LAST_ERROR=""; _TTS_RENDER_LAST_AT=utcnow_iso(); return data
        finally:
...
def _estimate_dispatch_ms(text: str, rate: float = 0.96) -> int:
    words = len([x for x in normalize_space(text).split(" ") if x])
    safe_rate = max(0.65, min(1.25, float(rate or 0.96)))
...
def play_dispatch_tts_on_host(text: str, rate: float = 0.96, volume: int = 100, cue_service: str = "brandweer", cue_urgent: bool = False, attention: bool = True) -> dict:
    ...
    with _TTS_PLAYER_LOCK:
        ...
        try:
            _TTS_PLAYER_PROCESS = subprocess.Popen([*argv, str(cache_file)], stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, creationflags=getattr(subprocess, "CREATE_NO_WINDOW", 0))
        except Exception as exc:
...
def github_update_config(config: dict) -> dict:
    ...
    return {
        ...
        "github_manifest_enabled": bool(config.get("github_manifest_enabled", True)) and bool(repo),
    }
...
def sanitize_github_update_payload(payload: dict, current: dict | None = None) -> dict:
    ...
    return {
        ...
        "github_manifest_enabled": bool(payload.get("github_manifest_enabled", current.get("github_manifest_enabled", True))) and bool(repo),
    }
...
def _github_latest_manifest(repo: str, branch: str) -> dict:
    """Read the tiny v4.5 update manifest before falling back to legacy branch ZIPs."""
    revision = _github_branch_head_public(repo, branch)
    body = _github_raw_file(repo, DEFAULT_GITHUB_MANIFEST_PATH, revision)
    doc = parse_manifest(body, APP_VERSION)
    platform_key = update_platform_key()
    row = doc["platforms"].get(platform_key)
    if not row:
        raise ValueError(f"Update-manifest bevat geen pakket voor {platform_key}")
    return {
        "repo": repo, "version": doc["version"], "tag": branch, "revision": revision,
        "name": f"P2000 manifest v{doc['version']} ({platform_key})", "published_at": doc.get("published_at"),
        "html_url": f"https://github.com/{repo}/blob/{revision}/{DEFAULT_GITHUB_MANIFEST_PATH}",
        "body": normalize_space(str(doc.get("notes") or "Manifest-gestuurde platformupdate."))[:4000],
        "source_kind": "manifest", "required": bool(doc.get("required")),
        "min_supported_version": str(doc.get("min_supported_version") or ""),
        "asset": {"name": row["name"], "url": row["url"], "size": int(row.get("size") or 0),
                  "digest": f"sha256:{row['sha256']}" if row.get("sha256") else ""},
    }


def _github_latest_software(repo: str, branch: str, include_branch: bool, include_manifest: bool = True) -> dict:
    candidates: list[dict] = []
    errors: list[str] = []
    if include_manifest:
        try:
            candidates.append(_github_latest_manifest(repo, branch))
        except urllib.error.HTTPError as exc:
            if int(getattr(exc, "code", 0) or 0) != 404:
                errors.append(f"Manifest: {_github_http_error(exc)}")
        except Exception as exc:
    ...
    # On equal versions prefer the signed/hash-checked platform manifest. Branch
    # remains the compatibility fallback for pre-v4.5 installations.
    source_rank = {"manifest": 3, "branch": 2, "release": 1}
    candidates.sort(key=lambda row: (_version_key(row.get("version", "")), source_rank.get(row.get("source_kind"), 0)), reverse=True)
    return candidates[0]
...
def _github_update_available(candidate: dict, current_version: str = APP_VERSION, installed: dict | None = None) -> tuple[bool, str]:
    ...
    if _version_key(candidate.get("version", "")) != _version_key(current_version):
        return False, ""
    if candidate.get("source_kind") not in {"branch", "manifest"}:
        return False, ""
...
def _download_github_asset(release: dict) -> Path:
    ...
    try:
        ...
        host = ""
    if host not in {"github.com", "objects.githubusercontent.com", "release-assets.githubusercontent.com", "codeload.github.com", "raw.githubusercontent.com"}:
...
def _backup_user_settings(reason: str = "update") -> Path:
    """Keep a tiny, human-readable settings backup independently of code rollback."""
    SETTINGS_BACKUP_DIR.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    target = SETTINGS_BACKUP_DIR / f"settings-{reason}-{stamp}.json"
    payload = {"created_at": utcnow_iso(), "reason": reason, "app_version": APP_VERSION, "config": {}}
    try:
        payload["config"] = json.loads(CONFIG_PATH.read_text(encoding="utf-8")) if CONFIG_PATH.exists() else {}
    except Exception:
        payload["config"] = {}
    try:
        if DB_PATH.exists():
            con = sqlite3.connect(DB_PATH)
            row = con.execute("SELECT value FROM settings WHERE key='display_settings'").fetchone()
            con.close()
            if row: payload["display_settings"] = json.loads(row[0])
    except Exception:
        pass
    target.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    rows = sorted(SETTINGS_BACKUP_DIR.glob("settings-*.json"), key=lambda x: x.stat().st_mtime, reverse=True)
    for old in rows[20:]: old.unlink(missing_ok=True)
    return target


def startup_selftest(config: dict) -> dict:
    """Fast, side-effect-light boot diagnostics used by launchers and the health page."""
    checks=[]
    def add(name, ok, detail="", critical=False): checks.append({"name":name,"ok":bool(ok),"detail":str(detail)[:500],"critical":bool(critical)})
    add("python", sys.version_info >= (3,10), sys.version.split()[0], True)
    add("backend", (BACKEND_DIR / "server.py").is_file(), str(BACKEND_DIR / "server.py"), True)
    add("frontend", (FRONTEND_DIR / "index.html").is_file() and (FRONTEND_DIR / "control.html").is_file(), str(FRONTEND_DIR), True)
    try:
        DATA_DIR.mkdir(parents=True, exist_ok=True); probe=DATA_DIR / f".write-{os.getpid()}"; probe.write_text("ok"); probe.unlink(missing_ok=True); add("data-writable", True, str(DATA_DIR), True)
    except Exception as exc: add("data-writable", False, exc, True)
    if os.name == "nt":
        browser = shutil.which("msedge.exe") or shutil.which("chrome.exe")
        add("windows-audio", bool(_powershell_executable()), "PowerShell/SAPI" if _powershell_executable() else "PowerShell ontbreekt")
    else:
        add("linux-audio", bool(shutil.which("piper") or shutil.which("espeak-ng") or shutil.which("espeak")), "piper/espeak")
    try:
        secret = SW_API_SECRET_PATH
        if secret.exists() and os.name != "nt": add("api-secret-permissions", (secret.stat().st_mode & 0o077) == 0, oct(secret.stat().st_mode & 0o777))
    except Exception as exc: add("api-secret-permissions", False, exc)
    result={"ok":not any((not x["ok"] and x["critical"]) for x in checks),"checks":checks,"created_at":utcnow_iso(),"safe_mode":SAFE_MODE}
    try: STARTUP_STATE_PATH.write_text(json.dumps(result,ensure_ascii=False,indent=2),encoding="utf-8")
    except Exception: pass
    return result
...
def github_check_and_maybe_install(state: "AppState", install: bool = False) -> dict:
    ...
    release = _github_latest_software(repo, cfg.get("github_branch") or DEFAULT_GITHUB_BRANCH, bool(cfg.get("github_branch_updates")), bool(cfg.get("github_manifest_enabled", True)))
...
def _validate_and_extract_update(zip_path: Path) -> tuple[Path, str]:
    # Never stage below the installation path on Windows. Downloads/Telegram can
    # already be very deep and the old updater hit MAX_PATH at exactly 260 chars.
    # A short OS temp path also makes updates independent of where the app lives.
    UPDATE_DIR.mkdir(parents=True, exist_ok=True)
    stage = Path(tempfile.mkdtemp(prefix="p2u-"))
    (stage / ".p2000-update-stage").write_text(APP_VERSION, encoding="utf-8")
    total = 0
    count = 0
    with zipfile.ZipFile(zip_path) as zf:
        ...
        for info in infos:
            ...
            # Combined/legacy GitHub branch archives may contain both platforms.
            # Do not even materialize the irrelevant platform: this prevents both
            # Windows path-length failures and accidental Linux-script installs.
            if os.name == "nt" and any(x == "linux" or "p2000_monitor_linux" in x for x in folded):
                continue
            if sys.platform.startswith("linux") and any(x == "windows" or "p2000_monitor_windows" in x for x in folded):
                continue
            ...
            if os.name == "nt" and len(str(target)) >= 245:
                raise ValueError(f"Updatepad blijft te lang voor Windows ({len(str(target))} tekens): {Path(name).name}")
    ...
    # Accept both a traditional single application folder and the v4.5.0
    # distribution layout: Bundle/Windows/... + Bundle/Linux/.... Limit the
    # recursive discovery depth so archived runtime backups can never become an
    # accidental update candidate.
    for server_file in stage.glob("**/backend/server.py"):
        root = server_file.parent.parent
        try:
            depth = len(root.relative_to(stage).parts)
        except ValueError:
            continue
        if depth > 3:
            continue
        if (root / "frontend" / "index.html").is_file() and (root / "frontend" / "control.html").is_file():
    ...
    if len(roots) == 1:
        ...
        # v4.5.0 bundles may deliberately contain separate Windows/ and Linux/
        # applications. During self-update pick only the folder for this host.
        preferred = "windows" if os.name == "nt" else "linux" if sys.platform.startswith("linux") else ""
        platform_roots = [r for r in roots if preferred and r.name.casefold() == preferred]
        if len(platform_roots) != 1:
            raise ValueError("Update bevat meerdere P2000-mappen maar geen eenduidige map voor dit platform")
        package_root = platform_roots[0]
    else:
        raise ValueError("ZIP bevat geen complete P2000 Monitor-map")
...
def _preflight_staged_update(package_root: Path, target_version: str) -> dict:
    ...
    proc=subprocess.Popen([sys.executable,str(server),"--bind","127.0.0.1","--port",str(port),"--no-poll","--safe-mode"],cwd=str(package_root),env=env,stdin=subprocess.DEVNULL,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,creationflags=(getattr(subprocess,"CREATE_NO_WINDOW",0) if os.name=="nt" else 0))
    output=b""
    try:
...
def _apply_update_and_exec(package_root: Path, target_version: str, install_meta: dict | None = None):
    try:
        ...
        _write_update_status(state="backup", target_version=target_version, message="Vorige versie en instellingen veiligstellen")
        settings_backup = _backup_user_settings("before-update")
        backup = _create_update_backup()
        _write_update_status(settings_backup=str(settings_backup))
        ...
        if isinstance(install_meta, dict) and install_meta.get("source_kind") in {"branch", "manifest"} and install_meta.get("revision"):
        ...
        try:
            ...
            while stage_dir != stage_dir.parent and not (stage_dir / ".p2000-update-stage").exists():
                stage_dir = stage_dir.parent
            if (stage_dir / ".p2000-update-stage").exists():
...
def _atomic_json_write(path: Path, payload: dict) -> None:
    """Durable replacement used by configuration and update recovery journals."""
...
def queue_supervisor_command(action: str) -> dict:
    ...
    _atomic_json_write(DATA_DIR/"supervisor-commands"/f"{time.time_ns()}-{row['token']}.json",row)
    return row
...
class Handler(BaseHTTPRequestHandler):
    ...
    def cors(self):
        origin=normalize_space(self.headers.get("Origin") or "")
        ...
        self.send_header("Access-Control-Allow-Headers", "Content-Type, X-P2000-Admin-Token")
    ...
    def do_POST(self):
        ...
        if not _same_origin_or_nonbrowser(self.headers):
        ...
        if parsed.path == "/api/remote/config":
            if not _REMOTE_ACCESS.is_loopback(client_ip):
                return self.send_json({"ok": False, "error": "Wijzig telefoontoegang op de lichtkrant-pc"}, 403)
            enabled = payload.get("enabled")
            if not isinstance(enabled, bool):
                return self.send_json({"ok": False, "error": "enabled moet true of false zijn"}, 400)
            with self.state.config_lock:
                try:
                    config = json.loads(CONFIG_PATH.read_text(encoding="utf-8")) if CONFIG_PATH.exists() else {}
                    config.update(bind="0.0.0.0" if enabled else "127.0.0.1")
        ...
        if parsed.path == "/api/displays/name":
            cid=normalize_space(str(payload.get("client_id") or ""))[:120]; name=normalize_space(str(payload.get("name") or ""))[:80]
            if not cid:return self.send_json({"ok":False,"error":"client_id ontbreekt"},400)
            with self.state.display_clients_lock:
                row=self.state.display_clients.get(cid)
                if row is None:return self.send_json({"ok":False,"error":"Scherm is niet bekend"},404)
                row["display_name"]=name or cid
            return self.send_json({"ok":True,"displays":self.state.display_clients_view()})
        ...
        if parsed.path == "/api/client-health":
            clean = {
                ...
                "display_name": normalize_space(str(payload.get("display_name") or ""))[:80],
                "fullscreen": bool(payload.get("fullscreen",False)),
                "display_mode": normalize_space(str(payload.get("display_mode") or ""))[:40],
                "user_agent": normalize_space(str(payload.get("user_agent") or ""))[:180],
                "current_incident_id": normalize_space(str(payload.get("current_incident_id") or ""))[:160],
            }
        ...
        if parsed.path == "/api/displays/test-target":
            target=normalize_space(str(payload.get("client_id") or ""))[:120]
            delivered,seq=self.state.publish_display_command({"type":"test","target_client_id":target,"payload":{"token":f"target-{int(time.time()*1000)}","mode":"message","title":"P 1 BR woning Teststraat Tilburg","city":"Tilburg","location":"Teststraat","service":"brandweer","priority":"P1","duration_ms":15000,"speak":False}})
            return self.send_json({"ok":True,"target_client_id":target,"command_seq":seq,"sse_delivered":delivered})
        ...
        if parsed.path == "/api/tts/play":
            # Last-resort Windows route for the local kiosk only. A LAN beheer-PC
            # must never make the backend machine speak accidentally.
            client_ip = self.client_address[0] if self.client_address else ""
            ...
            try:
                ...
                rate = 0.96
            try:
                result = play_dispatch_tts_on_host(
        ...
        if parsed.path == "/api/test-message":
            ...
            target_client_id=normalize_space(str(payload.get("target_client_id") or ""))[:120]
            delivered, command_seq = self.state.publish_display_command({"type": "test", "payload": test_payload, "target_client_id": target_client_id})
        ...
        if parsed.path == "/api/quick-action":
            ...
            if action in {"speech-normal","speech-mute","speech-priority"}:
                ...
                merged={"speechMode":mode,"speechEnabled":True}
            ...
            if action=="replay-last":
                ...
                return self.send_json({"ok":True,"clients":delivered,"queued":delivered == 0,"command_seq":_seq,"action":action})
    ...
    def handle_api(self, parsed):
        ...
        if parsed.path == "/api/runtime":
            ...
            return self.send_json({
                ...
                "startup_selftest": getattr(self.state, "startup_selftest", {}),
            })
        if parsed.path == "/api/setup":
        ...
        if parsed.path == "/api/displays":
            return self.send_json({"ok":True,"displays":self.state.display_clients_view(),"count":self.state.active_display_clients()})
        if parsed.path == "/api/health":
            return self.send_json({"ok": True, "health": self.state.health_snapshot()})
        if parsed.path == "/api/message-debug":
            mid=normalize_space(str(qs.get("id",[""])[0]))[:240]
            try:return self.send_json({"ok":True,"debug":message_debug_view(self.state,mid)})
            except ValueError as exc:return self.send_json({"ok":False,"error":str(exc)},404)
        if parsed.path == "/api/incident-corrections":
            return self.send_json({"ok":True,"corrections":_read_incident_corrections()})
        if parsed.path == "/api/test-presets":
            presets=[
              {"id":"fire","name":"Woningbrand","raw":"P 1 BR woning Hoofdstraat Tilburg","expect":{"service":"brandweer","tier":"normal"}},
              {"id":"middle-fire","name":"Middelbrand","raw":"P 1 MIDDEL BR woning Hoofdstraat Tilburg 209432","expect":{"service":"brandweer","tier":"high"}},
              {"id":"large-fire","name":"Grote brand","raw":"P 1 GROTE BR bedrijf Industrieweg Tilburg 209432 209431","expect":{"service":"brandweer","tier":"high"}},
              {"id":"police-bundle","name":"Politie letselnummer","raw":"P 1 386198 Letsel Beethovenlaan Tilburg","expect":{"service":"politie","forbid_unit":"386198"}},
              {"id":"a1-firewall","name":"A1 zonder brandweer","raw":"A1 Beethovenlaan Tilburg 209432","expect":{"service":"ambulance","forbid_unit":"209432"}},
              {"id":"baarle","name":"A1 Baarle-uitzondering","raw":"A1 BR assistentie Singel Baarle-Nassau 209432","expect":{"allow_fire":True}},
              {"id":"shooting","name":"Schietincident","raw":"P 1 Schietincident Ringbaan Tilburg","expect":{"tier":"critical"}},
              {"id":"mmt","name":"MMT inzet","raw":"A1 Reanimatie Hoofdstraat Tilburg 17992","expect":{"service":"lifeliner","tier":"critical"}}]
            return self.send_json({"ok":True,"presets":presets})
        ...
        if parsed.path == "/api/map-context":
            mid=normalize_space(str(qs.get("id",[""])[0]))[:240];zoom=bounded_int(qs.get("zoom",["14"])[0],14,7,18)
            try:return self.send_json({"ok":True,"context":map_context_view(self.state,mid,zoom)})
            except ValueError as exc:return self.send_json({"ok":False,"error":str(exc)},404)
        ...
        try:
            ...
            while not self.state.stop_event.is_set():
                try:
                    ...
                    with self.state.sub_lock:
                        if q not in self.state.subscribers:
                            break
...
def load_config() -> dict:
    default = {
        ...
        "github_manifest_enabled": True,
        "startup_selftest": True,
        "safe_mode_on_repeated_failure": True,
        "config_schema_version": CONFIG_SCHEMA_VERSION,
    }
    ...
    if CONFIG_PATH.exists():
        try:
            ...
            if isinstance(raw_loaded, dict):
                loaded, schema_changes = migrate_config(raw_loaded)
                if schema_changes:
                    try:
                        SETTINGS_BACKUP_DIR.mkdir(parents=True, exist_ok=True)
                        stamp=datetime.now().strftime("%Y%m%d-%H%M%S")
                        (SETTINGS_BACKUP_DIR/f"config-before-schema-{stamp}.json").write_text(json.dumps(raw_loaded,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
                    except Exception:
                        pass
    ...
    bind = normalize_space(str(default.get("bind") or "127.0.0.1"))
    default["bind"] = bind if len(bind) <= 120 else "127.0.0.1"
    ...
    default["github_manifest_enabled"] = default.get("github_manifest_enabled") is not False
    default["startup_selftest"] = default.get("startup_selftest") is not False
    default["safe_mode_on_repeated_failure"] = default.get("safe_mode_on_repeated_failure") is not False
    default["config_schema_version"] = CONFIG_SCHEMA_VERSION
    ...
    if (performance_migrated or schema_changes) and loaded:
        try:
            CONFIG_PATH.parent.mkdir(parents=True, exist_ok=True)
            tmp = CONFIG_PATH.with_suffix(".json.tmp")
            ...
            print(f"Waarschuwing: configmigratie kon niet worden opgeslagen: {exc}", file=sys.stderr)
    return default


# v4.5.5: landelijke grammatica-/fallbackparser. Fail-open: een defect in deze
...
# v4.5.7: fail-closed health/security/update hardening.
...
from remote_control import RemoteAccess
_REMOTE_ACCESS = RemoteAccess(DATA_DIR / "secrets" / "admin-token.txt")
def _v457_client_loopback(ip):
    return _REMOTE_ACCESS.is_loopback(ip)
def _v457_mutation_allowed(handler):
    return _REMOTE_ACCESS.authorized(handler)
...
def _v457_health_snapshot(self,force=False):
    ...
    with self.health_gate_lock:
        now = time.monotonic()
        if force or self.health_gate_cache is None or now - self.health_gate_at >= 8.0:
            self.health_gate_cache = _v457_eval_health(ROOT,expected_version=APP_VERSION)
            self.health_gate_at = now
        gate = self.health_gate_cache
...
def main():
    ...
    parser.add_argument("--safe-mode", action="store_true", help="Start without feed/update/TTS side effects for recovery")
    args = parser.parse_args()

    global SAFE_MODE, SAFE_MODE_REASON
    SAFE_MODE = bool(args.safe_mode or str(os.environ.get("P2000_SAFE_MODE", "")).lower() in {"1","true","yes"})
    SAFE_MODE_REASON = "handmatig gestart" if SAFE_MODE else ""
    config = load_config()
    if args.bind:
        config["bind"] = args.bind
    if args.port:
        config["port"] = args.port
    if SAFE_MODE or args.no_poll:
        config["github_auto_check"] = False
        config["github_auto_install"] = False
    boot_selftest = startup_selftest(config) if bool(config.get("startup_selftest", True)) else {"ok": True, "checks": [], "safe_mode": SAFE_MODE}
    ...
    if not args.no_poll and not SAFE_MODE:
    ...
    if not SAFE_MODE and not args.no_poll:
    ...
    if not SAFE_MODE and not args.no_poll and not read_sw_api_key():
    ...
    # Replace an old supervisor after a self-update before it interprets the new
    # backend version as a failure. Normal backend-only starts remain backend-only.
    if not SAFE_MODE and not args.no_poll and supervisor_status().get("running"):
        def reconcile_supervisor():
            if not state.stop_event.wait(1):
                ensure_supervisor_running()
        threading.Thread(target=reconcile_supervisor, daemon=True, name="supervisor-version-check").start()
