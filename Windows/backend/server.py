...
def parser_text_diagnostics(raw: str, parsed: dict, state: "AppState" | None = None) -> dict:
    ...
    # themselves stay out of speech, but the vehicle function is spoken. For a
    # tankautospuit we deliberately say "T S" so Windows/Linux TTS pronounce
    # the letters instead of silently reducing the dispatch to only the incident.
    if allowed and units:
        ...
        for unit in units[:12]:
            ...
            if code in {"TS", "TS-RES"} or code.startswith("TS-") or code == "TST-NB":
                base = "reserve T S" if code == "TS-RES" or "reserve" in label.lower() else "T S"
                if code in {"TST-NB"}: base = "T S natuurbrandbestrijding"
                elif code == "TS-WS": base = "T S met waterscherm"
                spoken = normalize_space(f"{base} {station}")
            elif code:
                expanded = {
                ...
                spoken = normalize_space(f"{expanded.get(code, label or code)} {station}")
            else:
                spoken = ""
        ...
        if spoken_units:
            if len(spoken_units) == 1:
                speech += f" Gealarmeerd voertuig: {spoken_units[0]}."
            elif len(spoken_units) == 2:
                speech += f" Gealarmeerde voertuigen: {spoken_units[0]} en {spoken_units[1]}."
            else:
                speech += f" Gealarmeerde voertuigen: {', '.join(spoken_units[:-1])} en {spoken_units[-1]}."
    if allowed:
...
def _validate_and_extract_update(zip_path: Path) -> tuple[Path, str]:
    ...
    # Accept both a traditional single application folder and the v4.4.18
    ...
    if len(roots) == 1:
        ...
        # v4.4.18 bundles may deliberately contain separate Windows/ and Linux/
